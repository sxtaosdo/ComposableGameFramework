...
export interface TileMatchHeadlessSampleResult {
  readonly snapshot: string;
  readonly restored: TileState;
  readonly renders: number;
  readonly shutdown: boolean;
}

/** Runs only when the Node headless sample explicitly invokes it. */
export function runTileMatchHeadlessSample(): TileMatchHeadlessSampleResult {
  ...
  runtime.deactivate();
  return {
    snapshot: serializeSnapshot(snapshot.value),
    restored: session.read(),
    renders: renders.length,
    shutdown: inputHandler === undefined,
  };
}
