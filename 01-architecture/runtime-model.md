...
扩展公开 `IState<T>` 和 `StateMachine<T>`。状态类由 `changeState(StateClass, enterParams)` 创建；状态机在离开当前状态后清除其 Context，再将同一 Context 注入新实例并等待其进入完成。`current` 只读返回当前状态，`is(StateClass)` 用于判断当前状态类型；二者不暴露内部存储。
...
`T` 是产品拥有的 Context。Framework 不读取或修改 Gameplay 状态；它可以是场景入口、Controller、Presenter 或其他产品对象，Framework 不规定其形态。Cocos `Node`、Prefab、UI 和业务规则不进入该 Runtime。
